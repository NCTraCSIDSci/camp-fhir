...
public class DocumentReferenceConversionTest {
	private static FhirContext ctx;
	private static DocumentReferenceConversion conversion;
	private SessionFactory sessionFactory;
	public static ResourceDao resourceDao;
	private static List<DocumentReference> fhirDocumentReferenceList = new ArrayList<DocumentReference>();
	private static List<main.java.com.campfhir.model.DocumentReference> dbDocumentReferences;
	...
	public void setup() throws IOException, IllegalAccessException, IllegalArgumentException, InvocationTargetException,
		...
		cf.setResource("DocumentReference");
		sessionFactory = HibernateBaseDB.getSessionFactory(cf, "in");
		Session session = sessionFactory.openSession();
		dbDocumentReferences = session.createQuery("from DocumentReference", main.java.com.campfhir.model.DocumentReference.class).list();
		session.close();

		conversion = new DocumentReferenceConversion();
		for (main.java.com.campfhir.model.DocumentReference DocumentReference : dbDocumentReferences) {
			DocumentReference fhirDocumentReference = conversion.DocumentReferences(DocumentReference);
			fhirDocumentReferenceList.add(fhirDocumentReference);
		}
