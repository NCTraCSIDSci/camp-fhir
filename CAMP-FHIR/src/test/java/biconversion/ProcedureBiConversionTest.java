...
import main.java.com.campfhir.service.bidirectionalconversion.ProcedureBidirectionalConversion;
import main.java.com.campfhir.dao.ResourceDao;

@TestInstance(TestInstance.Lifecycle.PER_CLASS)
public class ProcedureBiConversionTest {
	private static FhirContext ctx;
	public static ResourceDao resourceDao;
	private static String Procedurejson;
	private static Bundle resource;

	private static ProcedureBidirectionalConversion conversion = new ProcedureBidirectionalConversion();
	private static List<main.java.com.campfhir.model.Procedure> dbProcedures = new ArrayList<main.java.com.campfhir.model.Procedure>();
	private static List<org.hl7.fhir.r4.model.Procedure> fhirProcedureList = new ArrayList<org.hl7.fhir.r4.model.Procedure>();
	...
	public void setup() throws IOException, IllegalAccessException, IllegalArgumentException, InvocationTargetException,
		...
		Procedurejson = new String(Files.readAllBytes(Paths.get("./TESTING_DATA/Procedure.json")), StandardCharsets.UTF_8);
		resource = (Bundle) ctx.newJsonParser().setPrettyPrint(true).parseResource(Procedurejson);
		if (resource instanceof Bundle) {
			for (BundleEntryComponent entry : resource.getEntry()) {
				if (entry.getResource().getResourceType() == ResourceType.Condition) {
					fhirProcedureList.add(Procedure.class.cast(entry.getResource()));
					dbProcedures.add(conversion.Procedures(Procedure.class.cast(entry.getResource())));
				}
			}
		}
	}
